<script lang="ts">
  ...
  async function loadPagefind(): Promise<PagefindModule | null> {
    ...
    try {
      ...
      console.error('[@office-kit/docx search] failed to load pagefind index', err);
      status = 'unavailable';
      return null;
    }
  }
  ...
  // `/docs/recipes.html`), but SvelteKit's dev/preview servers only
  // resolve the canonical route (`/docs/recipes`). Strip the `.html`
